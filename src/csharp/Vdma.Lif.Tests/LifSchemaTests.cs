...
public class LifSchemaTests
{
    ...
    [Fact]
    public void FromFile_ValidJsonFile_ReturnsLifSchema()
    {
        var schema = LifJsonParser.FromFile(TestJsonFilePath);
    ...
    public void ToFile_ValidJsonFile_ContentSaved()
    {
        ...
        schema?.ToFile(fileName);

        var schemaFromFile = LifJsonParser.FromFile(fileName);

        AssertSchema(schemaFromFile);
    }

    private static void AssertSchema(LifSchema? schema)
    {
