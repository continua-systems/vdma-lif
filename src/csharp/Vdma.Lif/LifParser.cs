...
{
    /// <summary>
    ///     Reads a JSON file from the specified path and deserializes its content into an instance of <see cref="LifSchema" />
    ///     .
    /// </summary>
    /// <param name="jsonFilePath">The file path to the JSON file containing the data to be parsed.</param>
    /// <returns>
    ///     An instance of <see cref="LifSchema" /> representing the deserialized JSON data, or <c>null</c> if the
    ...
    /// </exception>
    public static LifSchema? FromFile(string jsonFilePath)
    {
    ...
    /// <summary>
    ///     Deserializes a JSON string into an instance of <see cref="LifSchema" />.
    /// </summary>
    ...
    ///     Thrown when the JSON data is invalid, incompatible with the <see cref="LifSchema" /> type,
    ///     or contains additional data beyond a single JSON value.
    /// </exception>
    /// <exception cref="NotSupportedException">
    ///     Thrown when there is no compatible <see cref="JsonConverter" /> for the <see cref="LifSchema" /> type
    ...
    ///     for the <see cref="LifSchema" /> type to convert.
    /// </exception>
    public static LifSchema? FromJson(string jsonData)
    {
        var options = new JsonSerializerOptions
        {
            PropertyNameCaseInsensitive = true
        };
        var schema = JsonSerializer.Deserialize<LifSchema>(jsonData, options);

        return schema;
    }

    /// <summary>
    ///     Serialize LIF schema into JSON string
    /// </summary>
    /// <param name="schema">LIF Schema</param>
    /// <param name="indented">Indicates whether JSON should use pretty printing</param>
    /// <returns>Serialized json string</returns>
    public static string ToJson(this LifSchema schema, bool indented = false)
    {
        var options = new JsonSerializerOptions
        {
    ...
    /// <summary>
    ///     Save LIF schema json serialized into file
    /// </summary>
    /// <param name="schema">LIF schema</param>
    /// <param name="filePath">file path</param>
    /// <param name="indented">Indicates whether JSON should use pretty printing</param>
    public static void ToFile(this LifSchema schema, string filePath, bool indented = false)
    {
        File.WriteAllText(filePath, schema.ToJson(indented));
    }
}